  ...
  const validateForm = () => {
    ...
    combinedPassangers.forEach((passanger) => {
      ...
      if (!passanger.issuingCountry)
        formErrors[`${passanger.type}-${passanger.index}-issuingCountry`] =
    ...
    setPassangerData((prevData) => ({
      ...
      [type]: prevData[type].map((passanger, idx) =>
        idx === index
          ? {
              ...passanger,
              showFamilyDataPassanger: !passanger.showFamilyDataPassanger,
            }
          : passanger
      ),
    }));
      ...
      if (flightDeparture?.id || flightReturn?.id) {
        ...
        try {
          ...
          if (flightIdReturn && flightReturn?.id) {
            ...
            console.log("Data Kursi Tersedia Pulang: ", availableSeatsReturn);
          }
        } catch (error) {
        ...
        <Row>
          <Col md={7}>
            ...
            <div style={{ paddingBottom: "40px" }}>
              ...
              >
                {seatsConfirmed ? "Booking berhasil dibuat" : "Simpan"}
              </Button>
            </div>
          </Col>
